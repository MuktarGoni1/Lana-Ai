    ...
    def _get_redis_client(self):
        ...
        if self._redis_client is None:
            try:
                ...
                self._redis_client = None
                self._redis_unavailable_until = now + 30
                if now - self._redis_last_error_at >= 30:
                    logger.warning("Redis unavailable; falling back to in-memory rate limiting: %s", e)
                    self._redis_last_error_at = now
                self._redis_client = None
    ...
    def check_rate_limit(
        ...
        if redis_client:
            ...
            try:
                ...
                redis_client.expire(key, window + 5)  # +5 seconds buffer
                ...
                self._redis_client = None
                self._redis_unavailable_until = current_time + 30
                if current_time - self._redis_last_error_at >= 30:
                    logger.warning("Redis error; falling back to in-memory rate limiting: %s", e)
                    self._redis_last_error_at = current_time
                pass
