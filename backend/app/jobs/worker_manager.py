...
class WorkerManager:
    ...
    async def start_workers(self) -> bool:
        ...
        if not settings.redis_url:
            logger.warning("REDIS_URL not configured; skipping job workers")
            return False
        ...
        try:
            try:
                ...
                logger.warning("Redis not reachable; skipping job workers: %s", e)
                return False
...
async def start_job_workers():
    ...
    return await worker_manager.start_workers()
