...
def get_current_user(
    ...
    if settings.api_secret_key not in insecure_defaults:
        try:
        ...
        logger.warning(
            "auth_hs256_disabled_insecure_secret", extra={"path": request.url.path}
        )
    ...
    if data is None and settings.supabase_url:
        ...
        try:
            ...
            data = jwt.decode(
                ...
                signing_key.key,
                algorithms=["RS256"],
                audience="authenticated",  # Supabase uses aud="authenticated"
            )
        except jwt.InvalidTokenError as e:
            logger.warning(
                "auth_rs256_decode_failed",
