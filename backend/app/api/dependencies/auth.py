...
def get_current_user(
    ...
    # First try local API secret (HS256) tokens only when explicitly configured.
    data = None
    hs256_secret = (settings.api_secret_key or "").strip()
    insecure_defaults = {"change_me_in_prod", "default_secret_key_change_in_production"}
    if hs256_secret and hs256_secret not in insecure_defaults:
        try:
            data = jwt.decode(token, hs256_secret, algorithms=["HS256"], options={"verify_aud": False})
        except Exception:
