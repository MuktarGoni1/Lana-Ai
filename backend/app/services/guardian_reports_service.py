...
class GuardianReportsService:
    ...
    async def generate_guardian_report(self, user_id: str, report_type: str = 'weekly') -> Dict[str, Any]:
        """
        ...
        try:
            ...
            user_events = await self._get_user_events(user_id, start_date, now)
            ...
            logging.error(f"Error generating guardian report for user {user_id}: {e}")
            raise
    ...
    async def get_guardians_for_report(self, report_type: str = 'weekly', user_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        ...
        """
        try:
            # Use guardian_settings as source of report recipients.
            column_name = f"{report_type}_report" if report_type in ['weekly', 'monthly'] else 'weekly_report'
            query = self.client.table("guardian_settings").select("user_id, email").eq(column_name, True)
            if user_id:
    ...
    async def save_guardian_report(self, user_id: str, guardian_email: str, report_type: str, 
        ...
        Args:
            user_id: The user ID the report belongs to
        ...
        try:
            ...
            logging.error(f"Error saving guardian report for user {user_id}: {e}")
            return None
