...
from backend.app.config import TTS_MODEL, TTS_SAMPLE_RATE, TTS_CHUNK_SIZE, TTS_CONCURRENT_LIMIT, TTS_CACHE_TTL
...
def test_tts_configurable_parameters():
    ...
    assert TTS_MODEL in {"gemini-2.0-flash-tts", "gemini-2.5-flash-preview-tts"}
...
def test_tts_service_initialization():
    ...
    with patch('backend.app.services.tts_service.genai') as mock_genai:
        mock_client = Mock()
...
def test_tts_concurrent_limiting():
    ...
    with patch('backend.app.services.tts_service.genai') as mock_genai:
        ...
        with patch('backend.app.config.TTS_CONCURRENT_LIMIT', 2):
