...
export async function GET(request: NextRequest) {
  try {
    console.log('[Debug Auth] Starting debug test');
    ...
    console.log('[Debug Auth] Initializing Supabase client');
    const adminClient = getSupabaseAdmin();
    console.log('[Debug Auth] Supabase client initialized');
    
    // Step 3: Test listUsers with minimal parameters
    console.log('[Debug Auth] Calling listUsers');
    const result = await adminClient.auth.admin.listUsers({
      page: 1,
      perPage: 1
    });
    
    console.log('[Debug Auth] listUsers result type:', typeof result);
    console.log('[Debug Auth] listUsers result keys:', result ? Object.keys(result) : null);
    
    // Check if result has data and error properties
    console.log('[Debug Auth] result.data type:', typeof result.data);
    console.log('[Debug Auth] result.error type:', typeof result.error);
    
    if (result.error) {
      console.log('[Debug Auth] Supabase error details:', {
    ...
    console.log('[Debug Auth] Data structure:', {
    ...
    if (Array.isArray(result.data.users) && result.data.users.length > 0) {
      console.log('[Debug Auth] First user structure:', {
