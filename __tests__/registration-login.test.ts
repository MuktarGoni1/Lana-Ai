...
describe('Registration and Login Flows', () => {
  ...
  // Test login flow
  describe('Login Flow', () => {
    test('verifies email authentication instead of sending magic link', async () => {
    ...
    test('handles login with unconfirmed email', async () => {
      global.fetch = jest.fn().mockResolvedValueOnce({
        ok: true,
        json: jest.fn().mockResolvedValueOnce({ exists: true, confirmed: false })
      }) as jest.Mock
      
      await expect(service.login('unconfirmed@example.com')).rejects.toThrow('Email not verified or confirmed')
    })
    ...
    test('handles login with non-existent email', async () => {
      ...
      await expect(service.login('nonexistent@example.com')).rejects.toThrow('Email not verified or confirmed')
    })
  })
