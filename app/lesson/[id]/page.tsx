...
export default function LessonPage() {
  ...
  async function loadOrGenerateLesson(topicId: string) {
    const unitRes = await authFetch(`/api/lesson/${topicId}`, { cache: "no-store" });
    ...
    if (unitRes.ok) {
      ...
      if (normalizedQuiz.length === 0) {
        const fetchQuizRes = await authFetch(`/api/quiz/${topicId}`, { cache: "no-store" });
        if (fetchQuizRes.ok) {
  ...
  async function beginGeneration(topicId: string, forceRefresh: boolean) {
    ...
    const startRes = await authFetch("/api/lesson/generate-job", {
      method: "POST",
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      if (stopped) return;
      try {
        const res = await authFetch(`/api/video/status/${videoJobId}`, { cache: "no-store" });
        if (!res.ok) return;
        ...
        await authFetch(`/api/lesson/${params.id}/video`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      if (stopped) return;
      try {
        const statusRes = await authFetch(`/api/lesson/generate-job/${generationJobId}`, { cache: "no-store" });
  ...
  async function fallbackGenerateLesson(topicId: string) {
    ...
    const lessonRes = await authFetch("/api/lesson/generate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const quizRes = await authFetch("/api/quiz/generate", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ topicId, forceRefresh: true }),
    });
  ...
  async function retryGeneration() {
    ...
    const res = await authFetch(`/api/lesson/${params.id}/regenerate`, {
      method: "POST",
    });
  ...
  async function submitQuiz() {
    ...
    if (score >= passScore) {
      await authFetch("/api/topic/complete", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
